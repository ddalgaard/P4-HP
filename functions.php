...
function redirect($url){
    /* Checks if headers are already sent, if not the function uses the good old header(Location: ) function, and if
    they are sent. The function first uses javascript to try and redirect, if javascript is disabled. The function uses
...
function addShift($startMonth, $endMonth, $startDate, $endDate, $startTime, $endTime, $workFunction, $empID, $notes){
    ...
    /* Note that $empID is not surrounded by quotes. This is because a NULL value can be inserted. If it was quoted,
    the null value would become a string. Quotes are added when necessary in the if-statement in browseDate.php. */
	$sql_query="INSERT INTO shift (shift_start, shift_end, skill_id, shift_emp_id, note)
	            VALUES ('$shiftStart', '$shiftEnd', '$workFunction', $empID, '$notes')";

    executeQuery($sql_query); 
}

// Function to update a shift.
function updateShift($startMonth, $endMonth, $startDate, $endDate, $startTime,
                     $endTime, $workFunction, $empID, $notes, $shiftID){
    ...
    /* Note that $empID is not surrounded by quotes. This is because a NULL value can be inserted.
    If it was quoted, the null value would become a string.
    Quotes are added when necessary in the if-statement in browseDate.php. */
    $sql_query ="UPDATE shift SET shift_start='$shiftStart', shift_end='$shiftEnd', skill_id='$workFunction',
                 shift_emp_id=$empID, note='$notes' WHERE shift_id='$shiftID'";
...
function checkIfEventExistOnDate($year, $month, $day){
    ...
    If no rows are returned no entry in the database is found for the specified date. */
...
function returnEventsOnDate($year, $month, $day){
    ...
    // http://www.w3schools.com/sql/func_datediff_mysql.asp
    // http://www.stillnetstudios.com/comparing-dates-without-times-in-sql-server/comment-page-1/
    /* The following select statement uses 2 left join because it works 3 different tables where the main table
    is shift. The joins helps to clarify the information in the shift table by adding skill name and employee
    name from skill and emp table. */
    $sql_query="SELECT shift.shift_start, shift.shift_end, shift.note, shift.shift_id, shift.skill_id,
                shift.shift_emp_id, CONCAT(emp.first_name, ' ', emp.last_name) AS name , skill.skill_name
...
/* Function to display and select workfunctions.
The value of each option is set to skill_id and the name of the value is the name of the skill. */
...
function createCalendar($month, $year){  
    ...
    /* Get the unix timestamp for the first day in the specified month.
    We use unix timestamp in order to make sure that the timestamp is not dependent on
    the server date/time, but the timezone of the visitor - using mktime(hour, minute, second, month, day, year). */
    $timestamp = mktime(0,0,0,$month,1,$year);
    
    /* Using the timestamp from the specified date,
    get the number of days in specified month - using date(format, timestamp). */
    ...
    /* Create table column headings (weekdays).
    Could as well have been made as an array where a foreach loop would grab each day from the array. */
    $calendar .='<thead>
                    <tr>
                        <th>Monday</th>
                        <th>Tuesday</th>
    ...
    Start count at 1 because monday is day in the week 1
    (we know this because $firstDayOfMonth returns '5' for the first day of March which is a friday). */
    ...
    for($dayInMonth = 1; $dayInMonth <= $numberOfDaysInMonth; $dayInMonth++){
        ...
        /* When the first day of month reaches 7,
        the row is ended to push days to a new row and the days counter is reset to start over again. */
...
function returnWeeklySchedule(){
    ...
    /* Get current day. Used to Generate a timestamp below. Because Sunday is the first day of the english week we
    subtract the date with one to 'fake' that monday is the first day of the week. E.g.: Sunday 7th will return monday
    8th as the first monday of that week. By subtracting 1 we really get the monday for last week which is correct
    ...
    /* Get monday in this week by using 'strtotime' and our timestamp. If we had not subtracted 1 from '$thisDay'
    this would have caused problems because the week would then start on sunday. */
    $mondayThisWeek = date("Y-m-d", strtotime("Monday this week",$timestamp));

    // Get the week start date (date for monday). 
    /* Used in the SQL query to extract events for each date. For each loop that extracts data,
    the date is added by 1 to get events for the next day. */
...
function returnHelloUser(){
    ...
    } // Echo the logout button which is displayed when hovering the user name.
}
...
function returnEventsOnID(){
    ...
    while($row = mysql_fetch_array($query_result)){
        ...
        //returnFormattedDateTime function is described beneath this function.
        echo "<tr> 
                <td>".returnFormattedDateTime($row['shift_start'])."</td>
                <td>".returnFormattedDateTime($row['shift_end'])."</td>
                <td>".$row['skill_name']."</td>
...
function returnFreeEvents(){
    ...
    /* Select statement that returns free shift from current date and forward, its limited to only show shifts that
    corresponds with the skills processed by the logged in employee. */
    $sql_query ="SELECT shift.shift_id, shift.shift_start, shift.shift_end, skill.skill_name, shift.note, shift.skill_id
                 ...
                 AND EXISTS (SELECT emp_id, skill_id FROM emp_skill WHERE emp_id = '$empID'
                             AND shift.skill_id = emp_skill.skill_id)
...
function addEmp($firstName, $lastName, $address, $zip_code, $email, $phone_no, $workFunction1, $workFunction2, $workFunction3, $isAdmin, $password){
    ...
    // Insert statements, divided into multiple statements in order to insert into different tables.
    ...
    $sql_7="INSERT INTO `login`(`emp_id`, `username`, `password`, `is_admin`) VALUES (LAST_INSERT_ID(),
            CONCAT('$firstName', LAST_INSERT_ID()),'$password', $isAdmin)";
    
    // Redefined the variable set by workFunction2 and 3, escaping special characters.
    ...
    mysql_query($sql_7);
        ...
        // Check if checkWorkFunction2 is set and preform a mysql_query if set.
        if ($checkWorkFunction2 > 0) {
            mysql_query($sql_5);
        }

        // Check if checkWorkFunction3 is set and preform a mysql_query if set.
...
function deleteEmp($emp_id) {
    ...
    // Redefined the variable set by emp_id and 3, escaping special characters.
    $checkEmp = mysql_real_escape_string($emp_id);

    // Check if checkEmp is set and preform the sql statements in the right order to delete an employee.
    if ($checkEmp > 0) {
...
function checkIfShiftsConflict($shiftId, $empId){
    ...
    $sql_query = "SELECT * FROM shift
                  WHERE (select shift_start from shift where shift_id = $shiftId) < shift_end
                  AND (select shift_end from shift where shift_id = $shiftId) > shift_start
                  AND shift_emp_id = $empId";

    // Execute query.
    $query_result = executeQuery($sql_query); 

    /* Get the amount of rows that is returned from the query. If one or more rows are returned an entry in the
    database is conflicting with the specified date interval and a warning is displayed. */
